...
-- Turtle Noise Blocks
noiseBlocks = {'minecraft:grass', 'botania:stone', 'minecraft:gravel', 'chisel:limestone', 'minecraft:dirt', 'minecraft:stone', 'minecraft:mossy_cobblestone', 'chisel:granite', 'chisel:andesite',  'chisel:diorite', 'chisel:marble', 'harvestcraft:salt'}
...
-- ********************************************************************************** --
function moveTurtle(moveFn, detectFn, digFn, attackFn, inspectFn, suckFn, maxDigCount, newX, newY, newZ)
    ...
    if (resuming == true) then
        ...
        if (lastMoveNeededDig == false) then
            ...
            if (lookForChests == true) then
                if (isNoiseBlock(inspectFn) == false) then
                    if (detectFn() == true) then
                        -- Determine if it is a chest before digging it
                        if (isChestBlock(inspectFn) == true) then
...
-- ********************************************************************************** --
function isNoiseBlock(inspectFn)
    ...
    for _, v in pairs(noiseBlocks) do
        if (v == data.name) then
            return true
        end
    end
...
-- ********************************************************************************** --
function isChestBlock(inspectFn)

    -- Compare block to inspect ID
    local success, data = inspectFn()
    if (success == false) then
        return false
    end
